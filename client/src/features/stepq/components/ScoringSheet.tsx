...
const sampleData: ScoreRow[] = [
	{ index: 1, answer: "A", correctAnswer: "A", score: 0 },
	{ index: 2, answer: "B", correctAnswer: "C", score: 0 },
	{ index: 3, answer: "D", correctAnswer: "D", score: 0 },
	{ index: 4, answer: "C", correctAnswer: "B", score: 0 },
	{ index: 5, answer: "A", correctAnswer: "A", score: 0 },
];

const ScoringSheet: React.FC = () => {
	// 選択状態: indexごとに 'correct' | 'incorrect' | undefined
	const [selected, setSelected] = useState<{ [key: number]: 'correct' | 'incorrect' | undefined }>({});
	const [rows, setRows] = useState<ScoreRow[]>(sampleData);
		...
		return (
			<div className="flex flex-col">
								...
								<tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
									{rows.map((row) => (
										<tr key={row.index} className="hover:bg-gray-100 dark:hover:bg-neutral-700">
											...
											<td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{row.answer}</td>
				...
				<div className="mt-4 text-right text-base font-semibold text-gray-700 dark:text-neutral-200">
					合計スコア: {rows.reduce((sum, row) => sum + row.score, 0)}
				</div>
			</div>
		);
	};
